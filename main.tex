...
L'intuizione di base a questo punto del progetto è quella di indagare sull'eventuale presenza di una correlazione tra la morfologia geometrica dei difetti e le propietà fisiche dei campioni di grafene da cui questi difetti derivano. 
...
Per analizzare i difetti individuati si è andati ad utilizzare una nota libreria di computer vision, OpenCV. Questa libreria offre numerosi metodi per sconotornare ed estsrarre parametri numerici sulla base di caratteristiche geometriche nell'immagine. 
...
Per fare questo si è fatto uso della funzione findContours di OpenCV, questo metodo è in grado di trovare contorni di oggetti su immagini.
Una volta trovati i contorni è bastato selezionare quello con il perimetro maggiore per selezionare il difetto ignorando tutto il resto. 
\begin{verbatim}
...
In questo modo stiamo evidenziando le zone di interesse eliminando tutte le altre cavità nel reticolo atomico che non devono essere  considerati come difetti.
...
\end{figure}
Il prossimo passo è stato quello di estrapolare i tratti geometrici del difetto stesso, sempre tramite metodi di OpenCV. Ne sono stati estratti numerosi, dai più semplici come perimetro e area fino a proprietà più sofisticate (circolarità, solidità, compattezza...).
In generale non è detto che tutte queste propietà saranno indispensabili per il modello predittivo finale, il lavoro da fare a questo punto sarà propio quello di individuare le relazioni causali fra una propietà e l'altra oltre che con il paraemtro target con lo scopo di individaure i tratti più significativi sui quali successivamente fare il training del modello.

Per fare tutto ciò, i dati raccolti sono stati integrati in un dataframe Pandas. Questa libreria offre metodi molto comodi per l'analisi di dati e sopratutto offre un oggetto chiamato dataframe molto comodo per ospitare i nsotri dati appena raccolti. 
Un dataframe Pandas si presenta che una tabella bidimensionale, simile a quelle di un foglio di calcolo, sulla quale si hanno a disposizione numere funzioni per manipolare, esplorare e visualizzare i dati. 

Prima di ottenere il dataframe finale sui quali sono state fatte le analisi è stato necessario risolvere un problema che ha a che fare con la natura delle immagini prodotte da YOLO. Quest'ultimo infatti per ogni difetto trovato in una delle immagini campioni originarie produce in generale più di una immagine, una per ogni difetto trovato. L'analisi tramite OpenCV e le propietà estsratte quindi sono relative in generale a un singolo difetto proveniente da un campione che ne può congtenere diversi. Il parametro total energy invece fa riferimento all'intero campione originale e non ai singoli difetti estratti da esso. Questo crea un problema in fase di correlazione tra propietà dei singoli difetti e paraemtro total energy. 
Per risolvere questo problema si è deciso di collassarare i parametri trovati dai singoli difetti in una sola riga del dataframe facendo la media pesata dei valori delle varie proprietà per l'area dei difetti. 
...
Di questi parametri quindi sarà sufficiente utilizzarne uno solo durante l'addestramento del modello predittivo, in quanto portano lo stesso contenuto informativo non è necessario utilizzarli entrambi. Questi tipo di ragionamenti nell'addestramento di modelli su dataset molto grandi può fare la differenza in quanto a tempo di addestramento.
\begin{center}
\begin{figure}[h]
\centering
...
\end{center}
\newpage
Nel lato alto a destra delle heatmap si confermano delle dipenze intuibili a priori, per esempio l'area e il perimetro correlatno quasi perfettamente. Questo è del tutto ragionevole in quanto in generale difetti con aree grandi avranno perimetri tentenzialmente grandi. 

Dopo la heatmap si è andati a generale un correlogramma che similmente alla heatmap mette in correlazione tutte le possibili coppie di properietà ma mostrandone anche l'andamento. 
\begin{center}
\begin{figure}[h]
\centering
...
\end{figure}    
\end{center}
I risultati estrapolabili da questo grafico sono molteplici. Si possono osservare correlazioni di vario tipo, dirette e inverse, lineari, logaritmiche, esponeniziali e anche relazioni più confuse dove non è chiaro se ci sia effettivamente una correlazione. 
Queste ultime pur se meno ovvie potranno dare comunque un contributo importante nell'addestramento del predittore. 
...
\begin{figure}[h]
\centering
\includegraphics[width=300px,keepaspectratio]{area_corr.png}
